...
namespace exec {
  ...
  class static_thread_pool {
    ...
    template <typename Sender, std::integral Shape, class Fun, std::enable_if_t<stdexec::sender<Sender>, int> = 0>
    using bulk_sender_t = //
      bulk_sender<
    ...
    template <class Fun, class Shape, class... Args>
      requires std::is_invocable_v<Fun, Shape, Args&...>
    ...
    struct domain {
      ...
      template <stdexec::__lazy_sender_for<stdexec::bulk_t> Sender, class Env>
        requires std::is_invocable_v<stdexec::get_scheduler_t, Env>
     ...
     private:
      ...
      class sender {
       ...
       private:
        ...
        template <typename Receiver, std::enable_if_t<stdexec::receiver<Receiver>, int> = 0>
      ...
      template <typename S, std::integral Shape, class Fn, std::enable_if_t<stdexec::sender<S>, int> = 0>
  ...
  struct static_thread_pool::bulk_sender {
    ...
    template <stdexec::__decays_to<bulk_sender> Self, typename Receiver, std::enable_if_t<stdexec::receiver<Receiver>, int> = 0>
      requires stdexec::

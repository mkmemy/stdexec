...
namespace exec {
  ...
  namespace __sequence_sndr {
    ...
    struct set_next_t {
      template <typename _Receiver, typename _Item, std::enable_if_t<sender<_Item> && receiver<_Receiver>, int> = 0>
  ...
  namespace __sequence_sndr {
    ...
    template <typename _Sigs, std::enable_if_t<__is_completion_signatures<_Sigs>, int> = 0>
    ...
    struct __stopped_means_break {
      struct __t {
        ...
        template <same_as<set_value_t> _SetValue, same_as<__t> _Self>
          requires std::is_invocable_v<set_value_t, _Receiver&&>
        ...
        template <same_as<set_stopped_t> _SetStopped, same_as<__t> _Self>
          requires std::is_invocable_v<set_value_t, _Receiver&&>
                && (unstoppable_token<_Token> || std::is_invocable_v<set_stopped_t, _Receiver &&>)
  ...
  concept __stoppable_receiver =                            //
    std::is_invocable_v<stdexec::set_value_t, _Receiver> && //
    (stdexec::unstoppable_token< stdexec::stop_token_of_t<stdexec::env_of_t<_Receiver>>>
     || std::is_invocable_v<stdexec::set_stopped_t, _Receiver>);

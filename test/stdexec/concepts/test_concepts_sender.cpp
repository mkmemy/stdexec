...
template <typename T, std::enable_if_t<!ex::sender<T> && !ex::sender_in<T, empty_env>, int> = 0>
not_a_sender_tag test_subsumption(T&&) {
  return {};
}

template <typename T, std::enable_if_t<ex::sender<T>, int> = 0>
sender_no_env_tag test_subsumption(T&&) {
  return {};
}

template <typename T, std::enable_if_t<ex::sender_in<T, empty_env>, int> = 0>
sender_env_tag test_subsumption(T&&) {
  return {};
}

template <typename T, std::enable_if_t<ex::sender_of<T, ex::set_value_t(), empty_env> && !ex::sender_in<T, empty_env>, int> = 0>

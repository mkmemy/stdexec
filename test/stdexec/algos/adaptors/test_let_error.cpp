...
TEST_CASE("let_error can be piped", "[adaptors][let_error]") {
  auto snd = ex::just() | ex::let_error([](std::exception_ptr) { return ex::just(); });
  (void) snd;
}
...
TEST_CASE("let_error of just_error with custom type", "[adaptors][let_error]") {
  ...
  auto snd = ex::just_error(my_type(&param_destructed)) //
...
TEST_CASE(
  ...
  auto s1 = ex::just_error(my_type(&param_destructed));
  auto snd = ex::just_error(my_type(&param_destructed)) //

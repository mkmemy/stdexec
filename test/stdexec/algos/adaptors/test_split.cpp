...
TEST_CASE("split doesn't advertise completion scheduler", "[adaptors][split]") {
  ...
  static_assert(!std::is_invocable_v<stdexec::get_completion_scheduler_t<ex::set_value_t>, snd_t>);
  static_assert(!std::is_invocable_v<stdexec::get_completion_scheduler_t<ex::set_error_t>, snd_t>);
  static_assert(
    !std::is_invocable_v<stdexec::get_completion_scheduler_t<ex::set_stopped_t>, snd_t>);
  (void) snd;
}

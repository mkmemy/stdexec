...
TEST_CASE("empty will complete immediately on an empty async_scope", "[async_scope][empty]") {
  ...
  auto snd = scope.on_empty() | ex::then([&] { is_empty = true; });
...
TEST_CASE("empty will complete after the work is done", "[async_scope][empty]") {
  ...
  bool is_empty{false};
  auto snd = ex::on(sch, scope.on_empty()) | ex::then([&] { is_empty = true; });
  auto op = ex::connect(std::move(snd), expect_void_receiver{});
  ex::start(op);
...
TEST_CASE("TODO: empty can be used multiple times", "[async_scope][empty]") {
  ...
  auto snd2 = ex::on(sch, scope.on_empty()) | ex::then([&] { is_empty2 = true; });
...
TEST_CASE("waiting on work that spawns more work", "[async_scope][empty]") {
  ...
  bool is_empty{false};
  auto snd = ex::on(inline_scheduler{}, scope.on_empty()) //
...
TEST_CASE(
  ...
  bool is_empty2{false};
  auto snd2 = ex::on(inline_scheduler{}, scope.on_empty()) //

...
TEST_CASE(
  ...
  auto snd = scope.spawn_future(
    ex::on(sch, ex::just() | ex::then([&] { executed = true; })));
  auto snd2 = std::move(snd) | ex::then([&] { REQUIRE(executed); });
...
TEST_CASE("spawn_future will start sender before returning", "[async_scope][spawn_future]") {
  ...
  {
    auto snd = scope.spawn_future(ex::just() | ex::then([&] { executed = true; }));
    (void) snd;
  }
  REQUIRE(executed);
  expect_empty(scope);
...
TEST_CASE(
  ...
  {
    auto snd1 = scope.spawn_future(ex::on(
      sch,
      ex::just() //
        | ex::let_stopped([&] {
            cancelled1 = true;
    ...
    auto snd2 = scope.spawn_future(ex::on(
      sch,
      ex::just() //
        | ex::let_stopped([&] {
            cancelled2 = true;
...
TEST_CASE(
  ...
  // TODO: make this work
  // auto snd = scope.spawn_future(ex::on(sch, ex::just_stopped()));
  // (void)snd;
  //
  // // The scope is now non-empty
  // REQUIRE_FALSE(P2519::__scope::empty(scope));
...
TEST_CASE("spawn_future forwards value to returned sender", "[async_scope][spawn_future]") {
  ...
  auto snd = scope.spawn_future(ex::on(sch, ex::just(13)));
  sch.start_next();
...
TEST_CASE("TODO: spawn_future forwards error to returned sender", "[async_scope][spawn_future]") {
  ...
  // TODO: fix this
  // auto snd = scope.spawn_future(ex::on(sch, ex::just_error(-1)));
  // sch.start_next();
  // try
  // {

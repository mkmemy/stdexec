...
TEST_CASE(
  ...
  "[adaptors][repeat_effect_until]") {
  auto source = just(1) | then([](int n) { return true; });
  auto snd = exec::repeat_effect_until(std::move(source));
...
TEST_CASE(
  ...
  "[adaptors][repeat_effect_until]") {
  auto snd = exec::repeat_effect_until(just(1));
  auto op = stdexec::connect(std::move(snd), expect_void_receiver{});
  start(op);
}
...
TEST_CASE(
  ...
  "[adaptors][repeat_effect_until]") {
  int n = 1;
  auto snd = exec::repeat_effect_until(just() | then([&n] {
                                                ++n;
                                                return n == 1'000'000;
                                              }));
  stdexec::sync_wait(std::move(snd));
